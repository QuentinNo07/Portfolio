document.addEventListener("DOMContentLoaded", () => {
    ...
    let degree = 2; // Par défaut pour la régression polynomiale
    ...
    regenerateButton.addEventListener("click", () => {
        ...
        representation.textContent = `Représentation : `;
    });
    ...
    function displayResults(result) {
        if (result) {
            afficher_equation.textContent = `Equation : ${result.equation}`;
            displayChart(result.predictions, data);
        }
    }

    // Affichage du graphique
    function displayChart(predictions, data) {
        const ctx = document.getElementById("myChart").getContext("2d");
        if (chart) {
            chart.destroy(); // Détruire le graphique précédent si existant
        }
        chart = new Chart(ctx, {
            type: 'scatter',
            data: {
                datasets: [
                    ...
                    {
                        label: 'Régression',
                        data: predictions.map(point => ({ x: point.x, y: point.y })),
